...
from .vae import StickBreakingVAE
...
class GSB:
    def __init__(self,bow_dim=10000,n_topic=20,taskname=None,device=None):
        ...
        self.vae = StickBreakingVAE(encode_dims=[bow_dim,1024,512,256,n_topic],decode_dims=[n_topic,512,bow_dim],dropout=0.0)
        self.device = device
        self.id2token = None
    ...
    def train(self,train_data,batch_size=256,learning_rate=1e-3,test_data=None,num_epochs=100,is_evaluate=False,log_every=5,beta=1.0,criterion='cross_entropy',ckpt=None,save_directory=None):
        self.vae.train()
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for iter,data in enumerate(data_loader):
                ...
                bows_recon,mus,log_vars = self.vae(bows)
    ...
    def inference_by_bow(self,doc_bow):
        ...
        with torch.no_grad():
            ...
            mu = torch.sigmoid(self.vae.fc1(mu))
            mu = torch.hstack([mu, torch.ones((mu.shape[0],1))])
